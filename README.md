...
- `test/acceptance/support-desk` - a runnable, end-to-end example: a fake
  ...
  performed via `attemptsTo`, same as anywhere else in Serenity/JS. `ticket`
  - the "question" - is a plain function instead, taking the actor and an
  optional `label` and returning its answer directly, called once the
  interactions that led up to it have completed. None of them reach into a
  fake domain's objects directly - they talk to
  [`system-under-test`](system-under-test), a small fake backend exposed
  ...
  `Ensure`/`equals` expectations checked against what `ticket` returns. A
  piece of context is an implementation detail of the interaction/question
  functions that put it on and read it off the `ScenarioContext` - not
  something a spec constructs or imports itself.
